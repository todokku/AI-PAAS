...
class SequenceDataHandler():
    ...
    def generate_train_data(self, unroll=True):
        ...
        if unroll == True:
            self._X_train, self._y_train = np.empty(
                [num_sequences, self._feature_size * self._sequence_length]), np.empty([num_sequences, num_classes])
        else:
            self._X_train, self._y_train = np.empty(
                [num_sequences, self._sequence_length, self._feature_size]), np.empty([num_sequences, num_classes])

        k = 0
        # Create the feature matrix by moving the sequence window for each sample
        for i in range(num_samples):
            for j in range(sequences_per_sample[i]):
                sequence_samples = self._X_train_list[i][
                                   j * self._sequence_stride:j * self._sequence_stride + self._sequence_length, :]

                if unroll == True:
                    self._X_train[k, :] = np.squeeze(sequence_samples.reshape(1,
                                                                              -1))  # If I dont squeeze I may be able to get the shape needed for RNN
                else:
    ...
    def generate_test_data(self, unroll=True):
        ...
        # Different shapes for the arrays depending wether data is to be unrolled or not
        if unroll == True:
            self._X_test, self._y_test = np.empty([num_samples, self._feature_size * self._sequence_length]), np.empty(
                [num_samples, num_classes])
        else:
            self._X_test, self._y_test = np.empty([num_samples, self._sequence_length, self._feature_size]), np.empty(
                [num_samples, num_classes])
    ...
    def generate_crossValidation_data(self, unroll=True):
        ...
        # Different shapes for the arrays depending wether data is to be unrolled or not
        if unroll == True:
            self._X_crossVal, self._y_crossVal = np.empty(
                [num_samples, self._feature_size * self._sequence_length]), np.empty([num_samples, num_classes])
        else:
            self._X_crossVal, self._y_crossVal = np.empty(
                [num_samples, self._sequence_length, self._feature_size]), np.empty([num_samples, num_classes])
