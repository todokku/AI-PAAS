...
class CMAPSSDataHandler(SequenceDataHandler):
    ...
    def __init__(self, data_folder, dataset_number, selected_features, max_rul, sequence_length=1, sequence_stride=1,
        ...
        self._column_names = {0: 'Unit Number', 1: 'Cycle', 2: 'Op. Settings 1', 3: 'Op. Settings 2',
                              4: 'Op. Settings 3', 5: 'T2',
                              6: 'T24', 7: 'T30', 8: 'T50', 9: 'P2', 10: 'P15', 11: 'P30', 12: 'Nf', 13: 'Nc',
                              14: 'epr', 15: 'Ps30',
                              16: 'phi', 17: 'NRf', 18: 'NRc', 19: 'BPR', 20: 'farB', 21: 'htBleed', 22: 'Nf_dmd',
                              23: 'PCNfR_dmd',
                              24: 'W31', 25: 'W32'}
        ...
        # super init
        super().__init__(sequence_length=sequence_length, sequence_stride=sequence_stride,
                         feature_size=len(selected_features), data_scaler=data_scaler)
    ...
    def generate_df_with_rul(self, df):
        ...
        # selected_features_rul.extend(['Cycle', 'Unit Number', 'RUL'])
    ...
    def create_dataFrames(self, verbose=0, cross_validation_ratio=0):
        ...
        if cross_validation_ratio < 0 or cross_validation_ratio > 1:
            print("Error, cross validation ratio must be between 0 and 1")
            return
        ...
        if self._data_scaler != None:
            ...
            norm_train_df = pd.DataFrame(self._data_scaler.fit_transform(self._df_train[cols_normalize]),
                                         columns=cols_normalize, index=self._df_train.index)
            ...
            # Rescale test data
            norm_test_df = pd.DataFrame(self._data_scaler.transform(self._df_test[cols_normalize]),
                                        columns=cols_normalize, index=self._df_test.index)
    ...
    def create_lists(self, cross_validation_ratio=0):
        ...
        self._X_train_list, self._y_train_list, self._X_crossVal_list, self._y_crossVal_list = self.generate_train_arrays(
    ...
    def load_data(self, unroll=True, cross_validation_ratio=0, verbose=0):
        ...
        if verbose == 1:
            print(
                "Loading data for dataset {} with window_size of {}, stride of {} and maxRUL of {}. Cros-Validation ratio {}".format(
